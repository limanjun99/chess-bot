...
inline Move uci::move(std::string_view uci_move, const Board& board) {
  ...
  const PieceVariant captured_piece{board.opp_player().piece_at(to)};
  if (uci_move.size() == 5) {
    const PieceVariant promotion_piece{piece_variant::from_char(uci_move[4])};
    return Move::promotion(from, to, promotion_piece, captured_piece);
  } else {
    const PieceVariant from_piece{board.cur_player().piece_at(from)};
