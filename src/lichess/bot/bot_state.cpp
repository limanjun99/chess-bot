...
void BotState::log_declined_challenge(const json& challenge) {
  Logger::info() << "Challenge to " << challenge["destUser"]["id"] << " from " << challenge["challenger"]["id"]
                 << " was declined because '" << challenge["declineReason"] << "'\n";
  Logger::flush();
}
...
void BotState::log_game_finish(const json& game) {
