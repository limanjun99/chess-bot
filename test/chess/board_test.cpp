...
TEST_CASE("kingside castling is applied to board properly") {
  ...
  REQUIRE(board.opp_player()[Piece::King] == bitboard::G1);
  REQUIRE(board.opp_player()[Piece::Rook] == (bitboard::A1 | bitboard::F1));
  board = board.apply_uci_move("e8g8");
  REQUIRE(board.opp_player()[Piece::King] == bitboard::G8);
  REQUIRE(board.opp_player()[Piece::Rook] == (bitboard::A8 | bitboard::F8));
}
...
TEST_CASE("queenside castling is applied to board properly") {
  ...
  REQUIRE(board.opp_player()[Piece::King] == bitboard::C1);
  REQUIRE(board.opp_player()[Piece::Rook] == (bitboard::D1 | bitboard::H1));
  board = board.apply_uci_move("e8c8");
  REQUIRE(board.opp_player()[Piece::King] == bitboard::C8);
...
TEST_CASE("pawn push does not wrongly capture en passant") {
  ...
  REQUIRE((board.cur_player()[Piece::Pawn] & bitboard::E5) == bitboard::E5);
}
...
TEST_CASE("promoting a pawn should remove the pawn from the board") {
  ...
  REQUIRE((board.opp_player()[Piece::Pawn] & bitboard::G8) == 0);
}