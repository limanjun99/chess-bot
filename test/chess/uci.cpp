...
TEST_SUITE("uci") {
  TEST_CASE("uci::move works for normal move") {
    ...
    REQUIRE(move.get_piece() == PieceVariant::Pawn);
    REQUIRE(move.get_captured_piece() == PieceVariant::None);
  }
  ...
  TEST_CASE("uci::move works for captures") {
    ...
    REQUIRE(move.get_piece() == PieceVariant::Pawn);
    REQUIRE(move.get_captured_piece() == PieceVariant::Pawn);
  }
  ...
  TEST_CASE("uci::move works for promotions") {
    ...
    REQUIRE(move.get_piece() == PieceVariant::Pawn);
    REQUIRE(move.get_captured_piece() == PieceVariant::None);
    REQUIRE(move.get_promotion_piece() == PieceVariant::Knight);
  }
}